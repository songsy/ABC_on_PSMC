...
#python /home/songsy/script/pop-gen/msmc_to_msHOT_YRI_MKK.py --file1 ../YRI/NA21302_gvcf_msmc.final.txt --file2 ../MKK/NA12878_1KG_msmc.final.txt --file3 ../African_MKK/psmcfa/YRI-MKK-psmc.2.txt
#python /home/songsy/script/pop-gen/msmc_to_msHOT_YRI_MKK.py --file1 ../YRI/NA21302_gvcf_msmc.final.txt --file2 ../MKK/NA12878_1KG_msmc.final.txt --file3 ../African_MKK/psmcfa/YRI-MKK-shapeit-psmc.3.txt
#python /home/songsy/script/pop-gen/msmc_to_msHOT_YRI_MKK.py --file1 ../YRI/NA19240_gvcf_msmc.final.txt --file2 ../MKK/NA21302_gvcf_msmc.final.txt --file3 ../African/psmcfa/YRI-MKK-psmc.0.txt
...
#		print time,size
	...
	cmds += '>sim_YRI_MKK_%i_%i.txt' %(t,m)
	print >>f_cmds,cmds
	print cmds
	cmds = 'python /home/songsy/script/pop-gen/make-psmc-from-ms-multiple.py sim_YRI_MKK_%i_%i.txt 2 %i both > sim_YRI_MKK_%i_%i_psmcfa.log' %(t,m,args.L,t,m)
	print >>f_psmcfa,cmds
	print cmds
	for i in range(0,3):
		cmds = 'psmc -N 25 -t 15 -r 5 -p "4+25*2+4+6" -o sim_YRI_MKK_%i_%i.%i.psmc sim_YRI_MKK_%i_%i.hetFQ.%i' %(t,m,i,t,m,i)
		print >>f_psmc,cmds
		print cmds
...
def msmc_to_msHOT_v2(file1,file2,file3,t,m,f_cmds,f_psmcfa,f_psmc):
	...
	pho = theta*0.13
	split_time = float(t*10000)/4/int(args.N0)/G
	check_point = float(137465.484312)/4/int(args.N0)/G
	print check_point
	migration_rate = m/1e05*4*int(args.N0)
	print split_time,migration_rate
	cmds = 'msHOT 4 %i -t %i -r %i %i -I 2 2 2 ' %(args.nrep,int(theta),int(pho),args.L)
...
#	print file1
	...
	for i in BedIterator(file3):   # only before the split, when two pop have shared history, also must before the checkpoint
		...
		if float(i[0])<137465.484312:
			continue
		if t*10000> 137465.484312:
...
#		cmds += '-ema 0.0025 2 0 %f %f 0 ' %(migration_rate,migration_rate)   # migration starting from 15kyr
		cmds += '-ema 0.001666667 2 0 %f %f 0 ' %(migration_rate,migration_rate)   # migration starting from 10kyr
		cmds += '-eM %f 0 ' %(split_time)
	cmds += '>sim_YRI_MKK_%i_%i.txt' %(t,m)
	print >>f_cmds,cmds
	print cmds
...
if __name__=="__main__":
	...
	parser.add_argument("--file2", dest='file2',help="MKK")
	parser.add_argument("--file3", dest='file3',help="African-vs-MKK")
