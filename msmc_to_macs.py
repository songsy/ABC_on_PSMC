...
#		cmds += '-ema 0.008333333 2 0 %f %f 0 ' %(migration_rate,migration_rate)   # migration starting from 50kyr
	...
	for i in range(args.nrep):
		new_cmds="%s-T 2>sim_macs_%s_CEU_%i_%i_%i_trees.txt 1>sim_macs_%s_CEU_%i_%i_%i_sites.txt" %(cmds,args.pop,t,m,i,args.pop,t,m,i)
...
def msmc_to_macs_v2(file1,file2,file3,t,m,f_cmds,f_psmcfa,migration_end,CHECKPOINT): # have exponential growth; all psmc input
	theta = 4*int(args.N0)*float(args.mu)*int(args.L)
	pho = theta*0.13
	split_time = float(t*10000)/4/int(args.N0)/G
	check_point = float(CHECKPOINT)/4/int(args.N0)/G
...
def msmc_to_macs_v4(file1,file2,file3,t,m,f_cmds,f_psmcfa,migration_end,CHECKPOINT): # have exponential growth 
	theta = 4*int(args.N0)*float(args.mu)*int(args.L)
	pho = theta*0.13
	split_time = float(t*10000)/4/int(args.N0)/G
	check_point = float(CHECKPOINT)/4/int(args.N0)/G
...
#	print file2
	...
	for i in range(len(interval2)):
		...
		cmds += '-en %f 2 %f ' %(interval2[i][0],interval2[i][1])
	cmds += '-ej %s 2 1 ' %(split_time)
	cmds += '-en %f 1 %f ' %(split_time+0.00001,share_interval[0][1])
	for i in range(len(share_interval)):
		cmds += '-en %f 1 %f ' %(share_interval[i][0],share_interval[i][1])
	if m!=0:
		migration_end = float(migration_end)*1000/G/4/args.N0
...
def msmc_to_macs_v5(file1,file2,file3,t,m,f_cmds,f_psmcfa,migration_end,CHECKPOINT):  # YRI_MKK, no growth
	theta = 4*int(args.N0)*float(args.mu)*int(args.L)
	pho = theta*0.13
	split_time = float(t*10000)/4/int(args.N0)/G
	check_point = float(CHECKPOINT)/4/int(args.N0)/G
...
#		cmds += '-ema 0.008333333 2 0 %f %f 0 ' %(migration_rate,migration_rate)   # migration starting from 50kyr
	...
	for i in range(args.nrep):
		new_cmds="%s-T 2>sim_macs_%s_LWK_%i_%i_%i_trees.txt 1>sim_macs_%s_LWK_%i_%i_%i_sites.txt" %(cmds,args.pop,t,m,i,args.pop,t,m,i)
		new_cmds = new_cmds.replace("SEED",str(1420845033+i*10))
		print >>f_cmds, new_cmds
#		msmc_cmds += 'sim_macs_MKK_CEU_%i_%i_%i_sites_2.txt ' %(t,m,i)
#	print >>f_psmc,msmc_cmds

def msmc_to_macs_v6(file1,file2,file3,t,m,f_cmds,f_psmcfa,migration_end,CHECKPOINT):  # YRI_MKK, no growth, use interval1_between
	theta = 4*int(args.N0)*float(args.mu)*int(args.L)
	pho = theta*0.13
	split_time = float(t*10000)/4/int(args.N0)/G
	check_point = float(CHECKPOINT)/4/int(args.N0)/G
	...
	for i in range(len(interval2)):
		...
		cmds += '-en %f 2 %f ' %(interval2[i][0],interval2[i][1])
	cmds += '-ej %s 2 1 ' %(split_time)
	print 'interval inbetween',interval1_inbetween
	for i in range(len(interval1_inbetween)):
		cmds += '-en %f 1 %f ' %(interval1_inbetween[i][0],interval1_inbetween[i][1])
	print share_interval
	for i in range(len(share_interval)):
...
def msmc_to_macs_GIH_CEU(file1,file2,file3,t,m,f_cmds,f_psmcfa,migration_end,CHECKPOINT): # have exponential growth 
	theta = 4*int(args.N0)*float(args.mu)*int(args.L)
	pho = theta*0.13
	split_time = float(t*10000)/4/int(args.N0)/G
	check_point = float(CHECKPOINT)/4/int(args.N0)/G
...
if __name__=="__main__":
	...
	parser.add_argument("--nrep", default=1000,dest='nrep',help="length of fragments")
		...
		for j in range(0,22,2):
			...
			if args.file3 is not None:
				if args.version is not None:
					...
					msmc_to_macs(args.file1,args.file2,args.file3,i,j,f_cmds,f_psmcfa,int(args.migration_end),int(args.checkpoint))
