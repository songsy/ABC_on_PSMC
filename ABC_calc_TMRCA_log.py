...
def msmc_to_macs_no_checkpoint_log(file1,file2,file3,t,m,migration_end,CHECKPOINT): # have exponential growth 
	theta = 4*int(args.simN0)*float(args.mu)*int(args.L)
	pho = theta*float(args.ratio)
	split_time = float(t*10000)/4/int(args.simN0)/G
	migration_rate = m/math.pow(10,m)*4*int(args.simN0)
	print t,m,split_time,migration_rate
	cmds = 'macs 4 %i -s SEED -t %f -r %f -I 2 2 2 ' %(args.L,theta/int(args.L),pho/int(args.L))
	growth = 0.0086848
...
#	print file1
	...
	growth_factor = -math.log(size_before_growth*int(args.simN0)/33813.99)/growth
	first = True
	interval1_beginning = []
	interval1_inbetween = []   # time interval after split time but before checkpoint 
	for i in BedIterator(file1):
...
def print_macs_cmds(iter,Ntrial,tprior,mprior):
	...
	for i in range(Ntrial):
		...
		cmds=msmc_to_macs_no_checkpoint_log(args.file1,args.file2,args.file3,t,m,int(args.migration_end),args.checkpoint)
...
def write_output_v2():
	for index in range(COPY):
		fout=open(args.psmc+'_TMRCA_ave_length_%s.txt' %(index),'w')
