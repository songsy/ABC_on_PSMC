...
import argparse,time,itertools,multiprocessing,math,glob,os,subprocess,pickle,sys,gzip
import numpy as np
...
def calc_f_density_complex_v6_explore_YRI_CEU(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_psuedo,checkpoint,migration_end):  # checkpoint is an index
	time_bin_list=[]
	migration_done=False
#	print t,m,NAf,NEu,migration_end
#	NEu_growth=5e04
#	growth_begin=3e04
	if migration_end==0:
		time_bin_list.append(time_bin(time=TMRCA_bin_psuedo.time_bin[0]*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu_growth*args.mu,m1=m1*1e-05/args.mu,m2=m2*1e-05/args.mu,label='after split'))
	else:
		...
		if growth_begin>float(migration_end):
			time_bin_list.append(time_bin(time=float(migration_end*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu_growth*args.mu,m1=m1*1e-05/args.mu,m2=m2*1e-05/args.mu,label='migration_end'))
			time_bin_list.append(time_bin(time=float(growth_begin*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1*1e-05/args.mu,m2=m2*1e-05/args.mu,label='after split'))
			...
			time_bin_list.append(time_bin(time=float(migration_end*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1*1e-05/args.mu,m2=m2*1e-05/args.mu,label='migration_end'))
		else:
			time_bin_list.append(time_bin(time=float(migration_end*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1*1e-05/args.mu,m2=m2*1e-05/args.mu,label='migration_end'))
	if TMRCA_bin_psuedo.time_bin[checkpoint]*args.G>=t*1e04:
		Anc_interval=checkpoint
...
def calc_f_density_complex_v6_explore_MKK_CEU_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_psuedo,checkpoint,migration_end):  # checkpoint is an index
	time_bin_list=[]
	migration_done=False
#	print t,m,NAf,NEu,migration_end
#	NEu_growth=5e04
...
def calc_f_density_complex_v6_explore_GIH_CEU_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_psuedo,checkpoint,migration_end):  # checkpoint is an index
	time_bin_list=[]
	migration_done=False
#	print t,m,NAf,NEu,migration_end
#	NEu_growth=5e04
#	growth_begin=3e04
	...
	m2=float(math.pow(10,-m2))
	if migration_end==0:
		time_bin_list.append(time_bin(time=TMRCA_bin_psuedo.time_bin[0]*args.mu,theta1=4*NEu_growth*args.mu,theta2=4*NEu_growth*args.mu,m1=m1/args.mu,m2=m2/args.mu,label='after split'))
	else:
		time_bin_list.append(time_bin(time=TMRCA_bin_psuedo.time_bin[0]*args.mu,theta1=4*NEu_growth*args.mu,theta2=4*NEu_growth*args.mu,m1=0,m2=0,label='after split'))
		if growth_begin>float(migration_end):
			time_bin_list.append(time_bin(time=float(migration_end*1e04)/args.G*args.mu,theta1=4*NEu_growth*args.mu,theta2=4*NEu_growth*args.mu,m1=m1/args.mu,m2=m2/args.mu,label='migration_end'))
			time_bin_list.append(time_bin(time=float(growth_begin*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1/args.mu,m2=m2/args.mu,label='after split'))
		elif growth_begin<float(migration_end):
			time_bin_list.append(time_bin(time=float(growth_begin*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=0,m2=0,label='after split'))
			time_bin_list.append(time_bin(time=float(migration_end*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1/args.mu,m2=m2/args.mu,label='migration_end'))
...
def calc_f_density_complex_v6_explore_YRI_MKK_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_psuedo,checkpoint,migration_end):  # checkpoint is an index
	time_bin_list=[]
	migration_done=False
#	print t,m,NAf,NEu,migration_end
#	NEu_growth=5e04
#	growth_begin=3e04
	...
	m2=float(math.pow(10,-m2))
	if migration_end==0:
		time_bin_list.append(time_bin(time=TMRCA_bin_psuedo.time_bin[0]*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1/args.mu,m2=m2/args.mu,label='after split'))
	else:
		time_bin_list.append(time_bin(time=TMRCA_bin_psuedo.time_bin[0]*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=0,m2=0,label='after split'))
		time_bin_list.append(time_bin(time=float(migration_end*1e04)/args.G*args.mu,theta1=4*NAf*args.mu,theta2=4*NEu*args.mu,m1=m1/args.mu,m2=m2/args.mu,label='migration_end'))
	if TMRCA_bin_psuedo.time_bin[checkpoint]*args.G>=t*1e04:
		Anc_interval=checkpoint
	else:
...
def calc_f_density_integrate_v3(tm_list,TMRCA_bin_psuedo,TMRCA_bin_Af,TMRCA_bin_Eu,index):  # fit 1 stats
	f_density,f_density11,f_density22=calc_f_density_complex_v6_explore_YRI_MKK(tm_list[index][0],tm_list[index][1],tm_list[index][2],tm_list[index][3],TMRCA_bin_psuedo,TMRCA_bin_Af,TMRCA_bin_Eu,args.checkpoint,args.migration_end)
	P=np.zeros(len(TMRCA_bin_psuedo.time_bin))
	t_bin=np.arange(0,0.0065,0.000001)
	for i in range(len(t_bin)):
		for j in range(len(P)-1):
...
def ABC_SMC_iter(iter,TMRCA_bin_psuedo,TMRCA_bin_Af,TMRCA_bin_Eu,error_value,prev_weight,prev_tm_list):
	...
	result_new=[]
	TMRCA_total =[]
	prior_list=[(args.tprior1,args.tprior2),(args.mprior1,args.mprior2),(args.NAfprior1,args.NAfprior2),(args.NEuprior1,args.NEuprior2)]
...
def plot_stats(iter,tm_list_total,tm_list_new,prev_tm_list,result_total,result_new,e,prev_weight,TMRCA_dist):
	...
	print 't,m',np.mean(t_accepted),np.mean(m_accepted)
	...
	NAf=[i[2] for i in tm_list_total]
	NEu=[i[3] for i in tm_list_total]
	xedges=list(np.linspace(args.NAfprior1,args.NAfprior2,num=21))
	yedges=list(np.linspace(args.NEuprior1,args.NEuprior2,num=21))
	heatmap, xedges, yedges = np.histogram2d(NAf,NEu,bins=(xedges,yedges))
	extents = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
	cax=ax3.imshow(heatmap.T,extent=extents,aspect="auto",interpolation='nearest',origin='lower',cmap='jet')
	fig.colorbar(cax,orientation='vertical')
	ax3.set_ylabel('NEu')
	NAf_accepted=[i[2] for i in tm_list_new]
	NEu_accepted=[i[3] for i in tm_list_new]
	print 'NAf,NEu',np.mean(NAf_accepted),np.mean(NEu_accepted)
	print heatmap
	ax4 = fig.add_subplot(414,title='iter %s accepted param' %(iter))
	heatmap, xedges, yedges = np.histogram2d(NAf_accepted, NEu_accepted,bins=(xedges,yedges))
	extents = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
	...
	ax3 = fig.add_subplot(413)
	cax=ax3.scatter(NAf_accepted,NEu_accepted,c=result_new,cmap=plt.cm.coolwarm)
	fig.colorbar(cax,orientation='vertical')
	...
	cax=ax4.scatter(t_accepted,m_accepted,c=new_weight,cmap=plt.cm.coolwarm)
...
if __name__=="__main__":
	...
	parser.add_argument("--NAfprior1", type=int,default=8,dest='NAfprior1',help="split time prior")
	parser.add_argument("--NAfprior2", type=int,default=15,dest='NAfprior2',help="split time prior")
	parser.add_argument("--NEuprior1", type=int,default=0,dest='NEuprior1',help="migration prior")
	parser.add_argument("--NEuprior2", type=int,default=20,dest='NEuprior2',help="migration prior")
	...
	parser.add_argument("--folder", type=str,dest='folder',help="folder")
...
#	TMRCA_bin_pseudo.calc_covariance()
	...
	prior_list=[(args.tprior1,args.tprior2),(args.mprior1,args.mprior2),(args.mprior1,args.mprior2),(args.NEuprior1*10,args.NEuprior2*10),(args.NAfprior1,args.NAfprior2),(args.NEuprior1,args.NEuprior2)]
	np.random.seed(args.random_number)
	tm_list=sample_parameters(args.Ntrial,prior_list)
#	os.popen('mkdir -p simulation')
	f=gzip.open('%s/sim_combo_%s_v3.gz' %(args.folder,args.random_number),'w')
	for index in range(args.Ntrial):
		t,m1,m2,NEu_growth,NAf,NEu=tm_list[index]
		growth_begin=np.random.uniform(3,5)
		if args.migration_end1 is not None:
			migration_end=np.random.uniform(args.migration_end1,args.migration_end2)
		elif args.migration_end is None:
			migration_end=np.random.uniform(0,4)
		else:
			migration_end=args.migration_end
		if args.name == 'YRI':
			f_density=calc_f_density_complex_v6_explore_YRI_CEU(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_pseudo,args.checkpoint,migration_end)
		elif args.name == 'MKK':
			f_density=calc_f_density_complex_v6_explore_MKK_CEU_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_pseudo,args.checkpoint,migration_end)
		elif args.name == 'GIH':
			f_density=calc_f_density_complex_v6_explore_GIH_CEU_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_pseudo,args.checkpoint,migration_end)
		elif args.name == 'YRI_MKK':
			f_density=calc_f_density_complex_v6_explore_YRI_MKK_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_pseudo,args.checkpoint,migration_end)
		else:
			f_density=calc_f_density_complex_v6_explore_MKK_CEU_log(t,m1,m2,NEu_growth,growth_begin,NAf,NEu,TMRCA_bin_pseudo,args.checkpoint,migration_end)
		P=np.zeros(len(TMRCA_bin_pseudo.time_bin))
		t_bin=np.arange(0,0.0065,0.000001)
		for i in range(len(t_bin)):
			for j in range(len(P)-1):
				if t_bin[i]>=TMRCA_bin_pseudo.time_bin[j]*args.mu and t_bin[i]<=TMRCA_bin_pseudo.time_bin[j+1]*args.mu:
					P[j]=P[j]+f_density[i]/sum(f_density)
					break
		print index
		print >>f,"%s\t%s" %("\t".join(map(str,tm_list[index]+[growth_begin,migration_end])),"\t".join(map(str,list(P))))
