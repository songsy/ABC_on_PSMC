...
def read_ped_v2(locus_dict,snp_ref,snp_alt,locus_present,sample_id,pop,pop_ind_num):
	...
	index=1
	for POP in ['CEU','GIH','LWK','MKK','YRI']:
		print POP
		index +=1
...
def read_store_locus(file):
	...
	for i in BedIterator(file):
		...
		strand[i[3]]=i[7]
		locus_dict[i[3]]=""
		locus_present[i[3]]=[0,0,0,0,0,0,0]
		if i[6]!=i[3]:
			hg19_to_hg18[i[6]]=i[3]
	pop_ind_num=[0,0,0,0,0,0,0]
...
def write_output(locus,sample_id,pop,locus_dict,locus_present,snp_ref,snp_alt,pop_ind_num):
	f_geno=open('Illumina_chip_1KG_phase3_HapMap.geno','w')
	f_snp=open('Illumina_chip_1KG_phase3_HapMap.snp','w')
	f_ind=open('Illumina_chip_1KG_phase3_HapMap.ind','w')
...
#	msmc_input_to_eigensoft(args.file)
	...
	dbfile=open('locus_dict_pickle','wb')
