...
def plot_stats(iter,new_tm_list,prev_tm_list,diff_genome_list,diff_chunk_list,prev_weight):
	fig = plt.figure()
...
#	fig.suptitle(sample_name, fontsize=10)
	...
	ax2.axvline(x=e,linewidth=2,color='k')
	accept_indicator=[1 if i<=e else 0 for i in diff_chunk_list]
	print 'iter%s error value:%f' %(iter,e)
...
#	plt.tight_layout()
	...
	fout = open('macs_iter%s_stats_weight.txt' %(iter),'w')
	for i in range(len(new_weight)):
		t=new_tm_list[i][0]
		m=new_tm_list[i][1]
		mean_diff_genome=diff_genome_list[i]
		mean_diff_chunk=diff_chunk_list[i]
		print >>fout,'%i\t%s\t%s\t%f\t%f\t%f' %(i,t,m,mean_diff_genome,mean_diff_chunk,new_weight[i])
	fout.close()
	return new_weight,e
...
def calc_stats_from_psmc_v2(iter,genome_percentage,chunk_percentage,interval_compare):
	f=open('macs_iter%s_param.txt' %(iter),'r')
#	fout = open('macs_iter%s_stats.txt' %(iter),'w')
	tm_list = []
	diff_genome_list=[]
	...
	for index,line in enumerate(f):
		...
		for i in range(len(TMRCA_bin1[0])):
			print >>f_stats,'%f\t%f\t%f\t%f' %(TMRCA_bin1[0][i],genome_percentage1[0][i],scale_factor1[0][i],chunk_percentage1[0][i])
			if i < interval_compare:
				a = 0.25*(genome_percentage[0][i]+genome_percentage[1][i]+genome_percentage[2][i]+genome_percentage[3][i])
				b = 0.25*(chunk_percentage[0][i]+chunk_percentage[1][i]+chunk_percentage[2][i]+chunk_percentage[3][i])
				diff_genome+=math.pow(a-genome_percentage1[0][i],2)
				diff_chunk+=math.pow(b-chunk_percentage1[0][i],2)
		mean_diff_genome=math.sqrt(diff_genome)
		mean_diff_chunk=math.sqrt(diff_chunk)
		f_stats.close()
#		print >>fout,'%i\t%s\t%s\t%f\t%f' %(index,t,m,mean_diff_genome,mean_diff_chunk)
		if index%100==0:
			print index,'done'
...
def get_stats_v2(iter,Ntrial,genome_percentage,chunk_percentage,interval_compare): # Use Euclidean distance this time
	f=open('macs_iter%s_param.txt' %(iter),'r')
	fout = open('macs_iter%s_stats.txt' %(iter),'w')
	tm_list = []
	diff_genome_list=[]
	...
	for index,line in enumerate(f):
		...
		for line_index,each_line in enumerate(f_stats):
			...
			if line_index > interval_compare-1:
				break
			a = 0.25*(genome_percentage[0][line_index]+genome_percentage[1][line_index]+genome_percentage[2][line_index]+genome_percentage[3][line_index])
			b = 0.25*(chunk_percentage[0][line_index]+chunk_percentage[1][line_index]+chunk_percentage[2][line_index]+chunk_percentage[3][line_index])
			diff_genome[j]+=math.pow(a-float(col[j*2+1]),2)
			diff_chunk[j]+=math.pow(b-float(col[j*2+2]),2)
		mean_diff_genome=math.sqrt(sum(diff_genome))
		mean_diff_chunk=math.sqrt(sum(diff_chunk))
		print >>fout,'%i\t%s\t%s\t%f\t%f' %(index,t,m,mean_diff_genome,mean_diff_chunk)
		if index%100==0:
			print index,'done'
		tm_list.append([float(t),float(m)])
...
if __name__=="__main__":
	...
	parser.add_argument("--last_iter", type=int,default=-1,dest='last_iter',help="last_iter")
	...
	for iter in range(ITERATION):
		...
		if iter==0:
			...
			new_tm_list,diff_genome_list,diff_chunk_list=calc_stats_from_psmc_v2(iter,genome_percentage,chunk_percentage,args.interval_compare)
			new_weight,error_value=plot_stats(iter,new_tm_list,prev_tm_list,diff_genome_list,diff_chunk_list,prev_weight)
		else:
