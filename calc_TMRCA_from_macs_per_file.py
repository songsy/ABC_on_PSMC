...
def get_TMRCA_from_macs(file,TMRCA_bin):
	...
	for i in range(args.nchr):
		macs_file=file+'_%s_sites.txt' %(args.start+i)
		new_file=file+'_%s_tree.txt' %(args.start+i)
		os.popen('grep NEWICK %s > %s' %(macs_file,new_file))
		f=open(new_file,'r')
		for line in f:
					...
					for j in range(int(args.nsample)):
						for hap in range(int(args.nhap)):
							...
							try:
								...
								print 'Error',dist,max(TMRCA_bin[index]),min(TMRCA_bin[index])
							if first[index] is True:
								prev_index_i[index]=index_i
								size_chunk[index]+=int(size)
...
def get_TMRCA_bin_v2(file,COPY):
	TMRCA_bin=[[] for x in range(COPY)]
	genome_percentage=[[] for x in range(COPY)]
	rho = [0 for x in range(COPY)]
	theta = [0 for x in range(COPY)]
...
def write_output_v7(file,TMRCA_bin_ind,genome_percentage_ind,chunk_percentage_ind,scale_factor_ind,copy,genome_percentage_bin,chunk_percentage_bin,average_length_bin):
	TMRCA = []
	for index in range(copy):
		fout=open(file+'_TMRCA_%s_new.txt' %(index),'w')
		for i in range(len(TMRCA_bin_ind[0])):
			output = '%f' %(TMRCA_bin_ind[index][i])
			output+='\t%f\t%f\t%f\t%f\t%f\t%f' %(genome_percentage_ind[index][i],float(chunk_percentage_ind[index][i])/sum(chunk_percentage_ind[index]),scale_factor_ind[index][i],float(genome_percentage_bin[index][i])/sum(genome_percentage_bin[index]),float(chunk_percentage_bin[index][i])/sum(chunk_percentage_bin[index]),average_length_bin[index][i])
			print >>fout,output
...
#	parser.add_argument("--N0", default=5e04*2.36/1.25,dest='N0',help="N0")
	parser.add_argument("--N0", type=int,default=5e04,dest='N0',help="N0")
	...
	parser.add_argument("--t", dest='t',help="split time")
	parser.add_argument("--m", dest='m',help="migration rate")
	args = parser.parse_args()


	if args.t!=None and args.m!=None:
		psmc_file = '%s_%s_%s_0.psmc' %(args.file,args.t,args.m)
		file = '%s_%s_%s' %(args.file,args.t,args.m)
	else:
		psmc_file = '%s_1.psmc' %(args.file)
		file = '%s' %(args.file)
	TMRCA_bin_ind,genome_percentage_ind,rho_ind=get_TMRCA_bin_single(psmc_file,1)
	chunk_percentage_ind,scale_factor_ind=calc_chunk_percentage_v2(genome_percentage_ind,TMRCA_bin_ind,rho_ind,1)
	genome_percentage_bin,chunk_percentage_bin,average_length_bin=get_TMRCA_from_macs_v3(file,TMRCA_bin_ind,args.start,1)
	write_output_v6(file,TMRCA_bin_ind,genome_percentage_ind,chunk_percentage_ind,scale_factor_ind,1,genome_percentage_bin,chunk_percentage_bin,average_length_bin)

#	psmc_file = 'sim_macs_Gravel_2pop_mig'
#	TMRCA_bin_ind,genome_percentage_ind,rho_ind=get_TMRCA_bin_v2(psmc_file,4)
#	chunk_percentage_ind,scale_factor_ind=calc_chunk_percentage_v2(genome_percentage_ind,TMRCA_bin_ind,rho_ind,4)
#	file = 'sim_macs_Gravel_2pop_mig'
#	genome_percentage_bin,chunk_percentage_bin,average_length_bin=get_TMRCA_from_macs_v3(file,TMRCA_bin_ind,args.start,4)
#	write_output_v7(file,TMRCA_bin_ind,genome_percentage_ind,chunk_percentage_ind,scale_factor_ind,4,genome_percentage_bin,chunk_percentage_bin,average_length_bin)
#	print psmc_file
