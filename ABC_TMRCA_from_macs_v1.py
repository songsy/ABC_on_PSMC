...
def get_TMRCA_from_macs_stdin(TMRCA_bin):
	TMRCA=[[] for x in range(COPY)]
	prev_index_i=[0 for x in range(COPY)]
	prev_TMRCA=[0 for x in range(COPY)]
	size_chunk=[0 for x in range(COPY)]
...
def get_TMRCA_from_macs_stdin_v2(file,TMRCA_bin,start):
	TMRCA=[[] for x in range(COPY)]
	prev_index_i=[0 for x in range(COPY)]
	prev_TMRCA=[0 for x in range(COPY)]
	size_chunk=[0 for x in range(COPY)]
...
def write_output_v3(genome_percentage_bin,chunk_percentage_bin):
	TMRCA = []
	fout=open(args.file+'_TMRCA_test3.txt','w')
...
def write_output_unnormalize(genome_percentage_bin,chunk_percentage_bin,index):
	TMRCA = []
	fout=open(args.file+'_TMRCA_%s.txt' %(index),'w')
	for i in range(len(TMRCA_bin[0])-1):
		TMRCA = [TMRCA_bin[index][i] for index in range(COPY)]
		output = '%f' %(np.mean(TMRCA))
		for index in range(COPY):
			output+='\t%f\t%f' %(genome_percentage_bin[index][i],chunk_percentage_bin[index][i])
		print >>fout,output
...
if __name__=="__main__":
	...
	parser.add_argument("--index", dest='index',help="prefix")
...
#	print 'get TMRCA from macs'
	...
	if args.index is not None:
		genome_percentage_bin,chunk_percentage_bin,average_length_bin=get_TMRCA_from_macs_stdin(TMRCA_bin)
		write_output_unnormalize(genome_percentage_bin,chunk_percentage_bin,args.index)
	else:
		print 'no index'
		genome_percentage_bin,chunk_percentage_bin,average_length_bin=get_TMRCA_from_macs_stdin_v2(args.file,TMRCA_bin,args.start)
		write_output_v3(genome_percentage_bin,chunk_percentage_bin)
#	os.popen('rm %s*sites.txt' %(args.file))
	if args.index is not None:
		os.popen('rm %s_%s_*trees.txt' %(args.file,args.index))
	else:
