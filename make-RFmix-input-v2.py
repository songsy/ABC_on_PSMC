...
def read_hapmap_ped():
	...
	hg19_alt={}
	for i in range(1,23):
		hg18_to_hg19['chr'+str(i)]={}
		hg19_alt['chr'+str(i)]={}
		hg19_ref['chr'+str(i)]={}
	for line in f_hg19:
		line = line.rstrip().split('\t')
		hg18_to_hg19[line[0]][int(line[6])]=int(line[2])
		hg19_ref[line[0]][int(line[2])]=line[4]
		hg19_alt[line[0]][int(line[2])]=line[5].split(',')
	index = 0
	pos_hg19=[]
	pos_chr=[]
	index_list=[]
	for line in f_map:
		...
		if int(line[0])>22:
			break
		try:
			hg19_pos=hg18_to_hg19['chr'+line[0]][int(line[3])]
		except KeyError:
			pos_hg19.append(0)
			pos_chr.append(0)
			rs_id.append(0)
		...
		pos_chr.append(line[0])   # 1-22
		index_list.append(index+6)
		index +=1
	print index,len(pos_hg19),len(pos_chr),len(index_list)
	CEU_list=[]
	YRI_list=[]
	...
	for i in BedIterator('/home/jmkidd/kidd-lab/genomes/snp-sets/hapmap3/PLINK/YRI_list.txt'):
		if i[1]=='0' and i[2]=='0':
			...
			print i
	print len(YRI_list),len(CEU_list)
	sample_list=[]
	pop_list=[]
	...
	for i in range(len(index_list)):
		index = index_list[i]-6
		pos = pos_hg19[index]
		chr = pos_chr[index]
		ref = hg19_ref['chr'+chr][pos]
		alt = hg19_alt['chr'+chr][pos]
		alt_allele=[]
		...
		if len(alt_allele)>1:
			...
			continue
		if len(alt_allele)==1:
			hg19_alt['chr'+chr][pos]=alt_allele
		alt = hg19_alt['chr'+chr][pos]
		m = []
		for j in range(snp_matrix.shape[0]):
			seq=snp_matrix[j,i]
	...
	snp = {}
	snp["rs"] = pd.Series([rs_id[k-6] for k in index_list],index=[pos_hg19[k-6] for k in index_list])
	snp["chr"] = pd.Series([pos_chr[k-6] for k in index_list],index=[pos_hg19[k-6] for k in index_list])
	snp["ref"] = pd.Series(ref_list,index=[pos_hg19[k-6] for k in index_list])
	snp["alt"] = pd.Series(alt_list,index=[pos_hg19[k-6] for k in index_list])
	snp["phase"] = pd.Series(haplotype,index=[pos_hg19[k-6] for k in index_list])
...
if __name__=="__main__":
	...
	SNP,sample_list,pop_list=read_hapmap_ped()
	...
	dbfile=open('hapmap_yri_ceu_pickle','wb')
	pickle.dump(SNP,dbfile) 
	for chr in range(1,23):
		interpolate,POS,MAP=genetic_map(chr)
		write_output(SNP,ind,chr,interpolate,POS,MAP)  	# for RFMix
		write_output_v2(SNP,ind,chr)			# for PCAdmix
