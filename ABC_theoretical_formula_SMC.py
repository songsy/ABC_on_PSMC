	...
	for i in range(1,len(time_bin_list)):
		if  time_bin_list[i].time < time_bin_list[i-1].time:
			for j in range(0,i+1):
				time_bin_list[j].print_time_bin()
			print 'wrong',i,time_bin_list[i].time,time_bin_list[i-1].time,t,m,NAf,NEu,migration_end
	T=float(t*1e04)/args.G*args.mu
#	print len(t_bin),T
	for i in range(len(t_bin)):
		if t_bin[i]<T:
...
#		result=[i[3]+i[4]+i[5] for i in result1]	# 3 stats
	...
	new_weight=plot_stats(iter,tm_list_total,tm_list_new,prev_tm_list,result_total,result_new,e,prev_weight,TMRCA_total,prior_list)
...
#	TMRCA_bin_Af.draw_TMRCA()
	...
	## PERFORM ABC
	if args.migration_end1 is not None or :
